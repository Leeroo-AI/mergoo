...
def convert_linear_to_moe(
    ...
    if (layer_idx in config.router_layers_index) \
        and (name in config.router_layers):
...
class LoRAMoeLayer(torch.nn.Module):
    def __init__(
        ...
        self.gate = torch.nn.Linear(config.hidden_size, config.num_experts, bias=False )#device="mps:0")# TODO FIXME
    ...
    def forward(self, x, *args, **kwargs):
        ...
        weights, selected_experts = torch.topk(gate_logits, self.num_experts_per_tok)# b,s,n
        weights = F.softmax(weights, dim=2, dtype=torch.float).to(previous_dtype)# b,s,n
        ...
        for ix, active_adapter in enumerate(self.active_adapters):
            ...
            batch_idx, tok_idx, expert_idx = torch.where(selected_experts == ix)
            x_adapter = x[batch_idx,tok_idx]# slicing uses the same tensor, whereas indexing will result in a copy. check the tensor address using tensor.storage().data_ptr()
            x_adapter = lora_B(lora_A(dropout(x_adapter))) * scaling # * self.config.global_scaling_weight
            # maybe we require a small linear layer that we train here, not sure.
            result[batch_idx,tok_idx] += x_adapter * \
