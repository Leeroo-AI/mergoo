...
class PhiMLP(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.fc1 = convert_linear_to_moe("fc1", config, layer_idx, config.hidden_size, config.intermediate_size, bias=True)
        self.fc2 = convert_linear_to_moe("fc2", config, layer_idx, config.intermediate_size, config.hidden_size, bias=True)
...
class PhiAttention(nn.Module):
    ...
    def __init__(self, config: PhiConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = convert_linear_to_moe("q_proj", config, layer_idx, self.hidden_size, self.num_heads * self.head_dim, bias=True)
        self.k_proj = convert_linear_to_moe("k_proj", config, layer_idx, self.hidden_size, self.num_heads * self.head_dim, bias=True)
        self.v_proj = convert_linear_to_moe("v_proj", config, layer_idx, self.hidden_size, self.num_heads * self.head_dim, bias=True)
        # check if changed to o_proj -> not being saved change to o_proj or dense in training config
        self.dense = convert_linear_to_moe("dense", config, layer_idx, self.num_heads * self.head_dim, self.hidden_size, bias=True)
...
class PhiDecoderLayer(nn.Module):
    def __init__(self, config: PhiConfig, layer_idx: int):
        ...
        self.mlp = PhiMLP(config, layer_idx)
...
class MOPPhiForCausalLM(PhiPreTrainedModel):
